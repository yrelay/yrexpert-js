...
EWD.application = {
  ...
  login: true,
  labels: {
    'ewd-title': 'Yexpert-JS - Andy',
    'ewd-navbar-title-phone': 'Yexpert-JS - Andy',
    'ewd-navbar-title-other': 'Yexpert-JS - Andy'
  },
  navFragments: {
    main: {
      cache: true
  ...
  onFragment: {
    ...
    'terminal.html': function(messageObj) {
      var url = 'https://' + $(location).attr('hostname') + ':' + EWD.YexpertTerminalPort + '/yrelay/yexpert-term/index.html';
  ...
  onMessage: {
    ...
    loggedIn: function(messageObj) {
      ...
      $('#main_Container').show();
      $('#mainPageTitle').text('Bienvenue sur Yexpert, ' + messageObj.message.name);
      $('#patientSelectionPanel').modal('show');
